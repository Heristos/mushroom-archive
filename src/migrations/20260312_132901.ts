...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`consoles\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`slug\` text NOT NULL,
  	\`icon\` text,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`consoles_slug_idx\` ON \`consoles\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`consoles_image_idx\` ON \`consoles\` (\`image_id\`);`)
  await db.run(sql`CREATE INDEX \`consoles_updated_at_idx\` ON \`consoles\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`consoles_created_at_idx\` ON \`consoles\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`games\` (
  	...
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`console_id\`) REFERENCES \`consoles\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`cover_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`games_console_idx\` ON \`games\` (\`console_id\`);`)
  await db.run(sql`CREATE INDEX \`games_cover_image_idx\` ON \`games\` (\`cover_image_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`categories_game_idx\` ON \`categories\` (\`game_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`items\` (
  	...
  	FOREIGN KEY (\`zip_file_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`icon_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`items_game_idx\` ON \`items\` (\`game_id\`);`)
  await db.run(sql`CREATE INDEX \`items_category_idx\` ON \`items\` (\`category_id\`);`)
  await db.run(sql`CREATE INDEX \`items_zip_file_idx\` ON \`items\` (\`zip_file_id\`);`)
  await db.run(sql`CREATE INDEX \`items_icon_idx\` ON \`items\` (\`icon_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`articles\` (
  	...
  	\`title\` text NOT NULL,
  	\`slug\` text NOT NULL,
  	\`author\` text DEFAULT 'MushArchive Team',
  	\`published_at\` text,
  	\`content\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL
  ...
  await db.run(sql`CREATE INDEX \`search_rels_games_id_idx\` ON \`search_rels\` (\`games_id\`);`)
  await db.run(sql`CREATE INDEX \`search_rels_items_id_idx\` ON \`search_rels\` (\`items_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`consoles_id\`) REFERENCES \`consoles\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`games_id\`) REFERENCES \`games\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_consoles_id_idx\` ON \`payload_locked_documents_rels\` (\`consoles_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`consoles\`;`)
